...
func OrderReceipt(ctx *gin.Context, orderId int64) (retErr error) {
	...
	if order.Status != constant.OrderStatus_Shipped {
		retErr = errors.Errorf("order status not match! order_id:%d, order_status:%d", order.OrderId, order.Status)
		return retErr
	}

	// 3. 更新订单状态
	err = dao.UpdateOrderStatus(ctx, tx, order.OrderId, constant.OrderStatus_Completed)
	if err != nil {
